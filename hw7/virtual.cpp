...
int main() {
	...
	if ((psy >> 7) == 0) {
		cout << hex << "To Disk Sector Address : " << (psy << 5) + psy_index << " contents: " << disk[psy][psy_index] << endl;
		return 0;
	}
	psy = psy & L;
	cout << hex << "To Physical Address : " << (psy << 5) + psy_index << " contents: " << page[psy][psy_index] << endl;
